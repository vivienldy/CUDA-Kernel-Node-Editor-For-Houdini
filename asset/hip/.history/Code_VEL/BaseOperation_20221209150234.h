    ...
    namespace Field
    {
        namespace GenericCode
        {
            ...
            __host__ __device__ inline glm::vec3 relbbox(CGGeometry::RawData file, glm::vec3 pos)
            {
                ...
                bbdelta[0] = (pos[0] - file.bbox.min[0]) / (file.bbox.max[0] - file.bbox.lowerBound[0]);
                bbdelta[1] = (pos[1] - file.bbox.min[1]) / (file.bbox.max[1] - file.bbox.lowerBound[1]);
                bbdelta[2] = (pos[2] - file.bbox.min[2]) / (file.bbox.max[2] - file.bbox.lowerBound[2]);
                return bbdelta;
            }
